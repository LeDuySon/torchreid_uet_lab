...
def evaluate(features, p_k, mAP_n, range, show = False, visualize_topk=5, infer_dir = None):
    ...
    print("Finish collect feature: ", end_collect - start)
    acc_top1 = calculate_rank_1(feat_list)
    top1_time = time.time()
    print("Rank 1: ", end_collect - top1_time)
    precision_k = calculate_precision_k(feat_list, p_k)
    precision_time = time.time()
    print("Precision time: ", precision_time - top1_time)
    mAP = calculate_mAP_n(feat_list, mAP_n)
    print("mAP time: ", time.time() - precision_time)
    ...
    print("precision_{}: {}".format(p_k, precision_k))
    print("mAP_{}: {}".format(mAP_n, mAP))
